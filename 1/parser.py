...
if len(sys.argv) > 1:
    ...
    if(len(list) == 0):
        ...
        for line in list:
            ...
            # print(line[-3])
            # print("<br/>")
            if line and line[len(line)-2] != '}' and line != "\n":
                fullString = fullString + line[:-2]
                continue
            else:
            ...
            for x in lists:
                if len(re.findall(x, line)) != 0:
                    ...
                    if x == REGULAR_ART: # если строка относится к названию статьи
                        art = deleteSpace(useRegular(REGULAR_CLEANING_SPACE, (re.findall(x, line))[0]))
